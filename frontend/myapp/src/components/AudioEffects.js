  ...
  const createReverbEffect = () => {
    ...
    for (let channel = 0; channel < impulse.numberOfChannels; channel++) {
      ...
      for (let i = 0; i < impulseLength; i++) {
        channelData[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / impulseLength, intensity / 50);
      }
    }
    ...
    return (
      <div style={styles.container}>
        ...
        <p style={styles.errorMessage}>
          Unable to initialize audio effects. The audio element may already be in use.
        </p>
      </div>
    );
  }
      ...
      <div style={styles.effectButtons}>
        {effects.map(effect => (
          <button
            ...
            style={{
              ...styles.effectButton,
              backgroundColor: activeEffect === effect.id ? effect.color : '#444',
